...
- Keep generated work in process memory for this PoC; it is not durable across API restarts.
...
Docker Compose is an optional local launcher: `docker compose up --build`. It serves the API on port 8000 and the Vite app on port 5173. Copy `.env.example` to `.env` and set `OPENAI_API_KEY` only when testing live generation; without it, the starter course remains available and generation routes return a structured configuration error.
